...
class SU2Model:
	...
	def __call__(self, pars):
		return expm(-1j * self.time * pars[0] * ( np.cos(pars[1])*np.cos(pars[2])*self.J[0] + np.cos(pars[1])*np.sin(pars[2])*self.J[1] + np.sin(pars[1])*self.J[2] ) )
...
model = Model(model=su2,npars=3,dH=3)

# ========= Statistical properties =========
STATISTICAL_ANALYSIS = True
if STATISTICAL_ANALYSIS:
	t = time()
	r = model.sampleAnalysis(sampleOverStates=True,sampleOverPars=True,parametersRange=np.array([[0,2*pi],[0,2*pi],[0,2*pi]]),
								Nstates=10,Npars=100,s=True,c=True,R=True,C_SLD=True,C_H=True,Delta=True)
