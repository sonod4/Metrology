...
# Ws = np.array([np.diag([1.,wi])/(1+wi) for wi in w])

r = model.fastSampleAnalysis(Nstates=Nstates,Npars=Npars,parametersRange=np.array([[0,2*pi],[0,2*pi]]),metrics=metrics,Ws=Ws,
							save=f"data/W2scrambler_{Ws.shape[0]}_{Nstates}_{Npars}.pkl")
