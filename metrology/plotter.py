...
class Plotter:
	...
	def plot3D(self, xlabel, ylabel, zlabel, filename=None, block=True):
		...
		ax.scatter(data[xlabel], data[ylabel], data[zlabel], marker='.')
	...
	def hist(self, xlabel, bins=100, ranges=None, filename=None, block=True):
		if not(filename is None):
			with open(filename, 'rb') as f:
				data = pkl.load(f)		
		else:
			data = self.data
		fig = plt.figure()
		plt.hist(data[xlabel], bins=bins, range=ranges, density=True)
	...
	def plotBounds(self,Nstates=1,Npars=1,metrics=["R","T"],sort_by=None,log=False,filename=None,block=True):
		...
		for idx in range(nplots):
			...
			if fix_states:
				...
				state = idx
				x = list(range( min(Npars,data[metrics[0]].shape[1]) ))  # or custom parameter labels
				ax.set_xlabel("Parameter index")
				ax.set_title(f"State {state}")
				if log:
					ax.set_yscale("log")
				if not(sort_by is None):
					order = np.argsort(data[sort_by][state,:Npars])
				...
				param = idx
				x = list(range( min(Nstates,data[metrics[0]].shape[0]) ))  # or custom state labels
				ax.set_xlabel("State index")
				ax.set_title(f"Parameter {param}")
				if log:
					ax.set_yscale("log")
				if not(sort_by is None):
					order = np.argsort(data[sort_by][:Nstates,param])
				for metric in metrics:
